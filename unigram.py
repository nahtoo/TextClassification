...
class Unigram_Baseline:
	...
	def __init__(self,training,training_classes,testing,testing_classes,output):
		...
		with open(output,"w") as output_file:
			naive_bayes_pipeline = Pipeline([('vect', CountVectorizer()),('clf', MultinomialNB())])
			...
			output_file.write(f"NB,UB,{nb_precision},{nb_recall},{nb_f1}\n")
			print("Naive bayes")
			print("F1:",nb_f1)
			...
			svm_pipeline = Pipeline([('vect', CountVectorizer()),('clf', SGDClassifier())])
			...
			output_file.write(f"SVM,UB,{svm_precision},{svm_recall},{svm_f1}\n")
			print("SVM")
			print("F1:",svm_f1)
			...
			logistic_pipeline = Pipeline([('vect', CountVectorizer()),('logistic', LogisticRegression())])
			...
			output_file.write(f"LR,UB,{logistic_precision},{logistic_recall},{logistic_f1}\n")
			print("Logistic")
			...
			random_forest_pipeline = Pipeline([('vect', CountVectorizer()) ,('Randomforest', RandomForestClassifier(max_depth=25))])
			...
			output_file.write(f"RF,UB,{rf_precision},{rf_recall},{rf_f1}\n")
			print("Random Forest")
			print("F1:",rf_f1)
	...
	def display_LC(self):
		...
		training_vector = Pipeline([('count', CountVectorizer())]).fit_transform(self.training+self.testing)
		...
		LG_train_sizes, LG_train_scores, LG_test_scores = learning_curve(estimator=LogisticRegression(),X=training_vector,y=combined_classes,train_sizes=selected_training_sizes,shuffle=True)
		...
		RF_train_sizes, RF_train_scores, RF_test_scores = learning_curve(estimator=RandomForestClassifier(max_depth=25),X=training_vector,y=combined_classes,train_sizes=selected_training_sizes,shuffle=True)
